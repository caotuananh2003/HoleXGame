...
public class GameplayPanel : UIWindow
{
    ...
    private void OnObjectiveUpdated(LevelObjective objective)
    {
        ...
        // Có fly effect: delay UpdateProgress và PlayCompleteAnimation đến khi icon đến nơi
        if (objectiveFlyEffect != null
            && objective.CurrentCount <= objective.RequiredCount
            && objective.ObstacleDefinition?.Icon != null
            && holeController != null)
        {
            // Báo cho ObjectiveManager biết có 1 animation đang chạy
            _gameplayObjectiveManager?.RegisterAnimationStarted();

            objectiveFlyEffect.Fly(
                objective.ObstacleDefinition.Icon,
                holeController.transform.position,
                item.RectTransform,
                onArrived: () =>
                {
                    OnIconArrived(objective, item);
                    // Báo animation đã xong
                    _gameplayObjectiveManager?.RegisterAnimationCompleted();
                });
        }
        else
        {
            // Fallback: không có fly effect thì cập nhật ngay
            ApplyObjectiveArrival(objective, item);
        }
    }

    private void OnObjectiveCompleted(LevelObjective objective)
    {
        // Intentionally empty — complete animation giờ được xử lý trong OnIconArrived
        // sau khi icon đã bay đến nơi xong.
    }

    /// <summary>
    /// Gọi khi icon fly đã đến nơi (hoặc fallback không có fly).
    /// Cập nhật text, rồi nếu vừa complete thì chạy animation biến mất.
    /// </summary>
    private void OnIconArrived(LevelObjective objective, ObjectiveUIItem item)
    {
        if (item == null) return;

        bool wasCompleted = !objectiveItemMap.ContainsKey(objective);
        if (wasCompleted) return; // đã bị remove khỏi map (complete animation đang chạy)
    ...
    private void ApplyObjectiveArrival(LevelObjective objective, ObjectiveUIItem item)
    {
        OnIconArrived(objective, item);
    }
